...
const ShippingAddress = ({ navigation }) => {
  const { user, token, updateCurrentUser } = useAppContext()
  ...
  const EmptyShippingAddress = () => {
    return (
      <View style={styles.emptyShippingAddressContainer}>
        <Ionicons
          name="location-outline"
          size={70}
          color={MATERIAL_COLORS.purple[600]}
        />
        <Text style={styles.message}>{`No Shipping address setup! \n Please set up your shipping address, by typing your post code, street address, zip code or any other address part into the search bar and press the enter key.`}</Text>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 8, paddingBottom: 0, backgroundColor: MATERIAL_COLORS.grey[200] }}>
        <SearchBar handleSelectedAddress={handleSelectedAddress} />
        {
          Object.values(fields).every((value) => (value === "" || value === 0)) && (
...
const styles = StyleSheet.create({
  ...
  button: {
    width: "95%",
    backgroundColor: MATERIAL_COLORS.blue[500],
    paddingHorizontal: 20,
    paddingVertical: 10,
    borderRadius: 30,
    marginTop: 8,
