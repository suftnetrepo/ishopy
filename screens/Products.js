...
const Products = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchAndLoadProducts = async () => {
      try {
        ...
        const { data, success } = await fetchProducts(null, VERBS.GET, null, queryParams);
  ...
  }, [seller, currentPage, category, searchTerm]);
  ...
  const renderItem = ({ item }) => {
    return (
      <View style={styles.card}>
        ...
        <View style={{
          ...styles.rowContainer, justifyContent: 'flex-end', marginTop: 8, position: 'absolute',
          top: 1,
          right: 4,
        }}>
          <StyledCycle
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <StyledSearchBar onReset={() => handleReset()} placeholder="Search by product name" onPress={(query) => setSearchTerm(query)} />
